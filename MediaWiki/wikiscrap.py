...
for x in sys.stdin:
	print "<doc>"
	print "{}{}{}".format("<field name=\"id\">", i, "</field>")

#tytuł strony
	for x in sys.stdin:
		wyn = re.search(r'^<title>.*– Wikipedia, wolna encyklopedia</title>$', x)  
		if wyn:
			wyn1 = re.sub(r' – Wikipedia, wolna encyklopedia', "", wyn.group())
			wyn2 = re.sub(r'<title>', "", wyn1)
			title = re.sub(r'</title>', "", wyn2)
...
#url	
	...
	#treść artykułu		
	for x in sys.stdin:
		pocz = re.search(r'"//meta.wikimedia.org"', x)
		if pocz:
			for x in sys.stdin:
				...
				if not(kat):
					...
					break
			#print "<field name=\"content\">" + re.sub(r'<.*?>', "", content) + "</field>"
			print "<field name=\"content\">" +  ''.join(BeautifulSoup(content, from_encoding = "utf-8").findAll(text = True)) + "</field>"
			...
			for word in miasta:
				...
				freq[word] = len(wyn)
			city = max(freq, key=freq.get)
			if not (freq[city]==0):
				print "<field name=\"city\">" + city + "</field>"
			break
	...
	for x in sys.stdin:
		if kat:
			...
			if wyn0:
				print "<field name=\"category\">miasto</field>"
			wyn1 = re.search(r'Kategoria:Kina', kat.group())
			if wyn1:
				print "<field name=\"category\">kino</field>"
			wyn2 = re.search(r'Kategoria:Teatry', kat.group())
			if wyn2:
				print "<field name=\"category\">teatr</field>"
			...
			if (wyn4 or wyn5 or wyn6 or wyn7 or wyn8):
				print "<field name=\"category\">obiekt sportowy</field>"
			wyn9 = re.search(r'Kategoria:Hotele', kat.group())
			if wyn9:
				print "<field name=\"category\">nocleg</field>"
			...
			if (wyn18 or wyn19 or wyn20):
				print "<field name=\"category\">zamek, pałac</field>"
			win30 = re.search(r'Kategoria:Parki_', kat.group())
			if win30:
				print "<field name=\"category\">park narodowy, krajobrazowy</field>"
			break
