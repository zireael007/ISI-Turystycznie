...
for x in sys.stdin:
	...
	for x in sys.stdin:
		wyn = re.search(r'^<title>.*– Wikipodróże, wolny przewodnik turystyczny</title>$', x)  
		if wyn:
			title = re.sub(r' – Wikipodróże, wolny przewodnik turystyczny', "", wyn.group())
			title = re.sub(r'<title>', "<field name='title'>", title)
			title = re.sub(r'</title>', "</field>", title)
			print title
			break
